...
$OsRoot = 'https://files.rg-adguard.net/version/f0bd8307-d897-ef77-dbd6-216fefbe94c5'
...
function Select-OutDir {
    try {
        ...
        $input = Read-Host
        if ([string]::IsNullOrWhiteSpace($input)) { return $defPath }
        return $input.Trim()
    } catch { return 'D:\WinIso' }
}

if (-not $OutDir)   { $OutDir = Select-OutDir }
if (-not $ToolsDir) { $ToolsDir = Join-Path $OutDir 'bin' }
...
function Show-Leaf {
    ...
    Write-Host ''
    if ($all.Count -gt 1) {
        Write-Warn2 "提示：rg-adguard 把它打包成一个差分组，共 $($all.Count) 个文件（先只下这一个，缺基准才补下其它）。"
    }
...
Write-Info "rg-adguard 镜像下载器（简版：操作系统 · 简体中文 · 自动 D）"
